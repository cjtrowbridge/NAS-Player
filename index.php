...
function NASPlayer($Path){
  ...
  $RootPath       = '/var/www/nas';
  $ClientRootPath = '/nas';

  $RelativePath = str_replace($RootPath,'',$Path);
  $RelativePath = rtrim($RelativePath,'/').DIRECTORY_SEPARATOR;

  if(is_file($RootPath.DIRECTORY_SEPARATOR.$Path)){
    echo '<video controls autoplay preload><source src="'.$ClientRootPath.$Path.'" type="video/mp4"></video><br><br><br><p>If you are missing codecs, the video won\'t display. You can also <a href="'.$ClientRootPath.$Path.'">click here</a> to download the file and try playing it locally. Depending on your browser, you may be able to cast this page even if it is not showing up, and have it work on the TV.</p>';
    return;
  }
  ...
  $Directories=array();
  $Files=array();
  if($Handle = opendir($RootPath.DIRECTORY_SEPARATOR.$Path)){
    while(false !== ($Entry = readdir($Handle))){
      if(is_dir($RootPath.DIRECTORY_SEPARATOR.$Path.DIRECTORY_SEPARATOR.$Entry)){
        if(($Entry !== '.') && ($Entry!=='..') && ($Entry!=='NAS-Player') && (substr($Entry, 0, 1) != '.')){
  ...
  foreach($Directories as $Name => $Directory){
    echo '<p><a href="./?path='.$RelativePath.str_replace('%20','+',urlencode($Name)).'">[D] '.$Name.'</a></p>';
  }
  foreach($Files as $Name => $File){
    echo '<p><a href="./?path='.$RelativePath.str_replace('%20','+',urlencode($Name)).'">[F] '.$Name.'</a></p>';
  }
}
...
function PathButtons($Path){
  ...
  foreach($Paths as $Path){
    echo ' / <a href="./?path='.$Done.$Path.'">'.$Path.'</a>';
    $Done.=$Path.DIRECTORY_SEPARATOR;
  }
}
    ...
    <div class="container">
      <div class="row">
       <div class="col-12">
         ...
         <hr>
         <p><i><a href="./?path=/">NAS</a>: <?php PathButtons($Path); ?></i></p>
         <hr>
       </div>
      </div>
      <div class="row">
